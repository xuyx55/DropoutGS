...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self.edge_xyz_gradient_accum = torch.empty(0)
        self.denom = torch.empty(0)
        self.edge_denom = torch.empty(0)
        self.optimizer = None
    ...
    def restore(self, model_args, training_args=None):
        ...
        self.edge_xyz_gradient_accum = edge_xyz_gradient_accum
        self.denom = denom
        self.edge_denom = edge_denom
    ...
    def training_setup(self, training_args):
        ...
        self.edge_xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.edge_denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def prune_points(self, mask):
        ...
        self.edge_xyz_gradient_accum = self.edge_xyz_gradient_accum[valid_points_mask]

        self.denom = self.denom[valid_points_mask]
        self.edge_denom = self.edge_denom[valid_points_mask]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, opa_thresh, N=2, edge_rate = None, edge_threshold = 1000, size_threshold_rate=1):
        ...
        if edge_rate is not None:
            # split large gaussian with large opacity and large edge rate
            edge_rate_new = torch.zeros((n_init_points), device="cuda")
            edge_rate_new[:edge_rate.shape[0]] = edge_rate.squeeze()
            edge_mask = edge_rate_new >= edge_threshold
            edge_mask = torch.logical_and(edge_mask, torch.max(self.get_scaling, dim=1).values > self.percent_dense * scene_extent * size_threshold_rate)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, opa_thresh, max_dist, edge_threshold = 1000, size_threshold_rate = 1):
        if max_dist is not None:
            outliers_mask = torch.sqrt(distCUDA2(self._xyz.detach()).view(max_dist.shape)) > max_dist
            self.prune_points(outliers_mask.squeeze())
        
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0
        edge_rate = self.edge_xyz_gradient_accum/(self.edge_denom + 1e-5)
        edge_rate[edge_rate.isnan()] = 0.0
        self.densify_and_clone(grads, max_grad, extent, opa_thresh)
        self.densify_and_split(grads, max_grad, extent, opa_thresh, edge_rate = edge_rate, edge_threshold = edge_threshold, size_threshold_rate = size_threshold_rate)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, pixels, imgrad_pixel, accum_denom=True, add_random=False, ):
        ...
        if pixels is not None and imgrad_pixel is not None:
            self.edge_xyz_gradient_accum[update_filter] += imgrad_pixel[update_filter]
            self.edge_denom[update_filter] += pixels[update_filter]
